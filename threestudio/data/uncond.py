...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        elevation_deg: Float[Tensor, "..."]
        elevation: Float[Tensor, "..."]
        if random.random() < 0.5:
            # sample elevation angles uniformly with a probability 0.5 (biased towards poles)
            elevation_deg = (
                torch.rand(self.batch_size) * (self.elevation_range[1] - self.elevation_range[0])
        ...
        if self.cfg.batch_uniform_azimuth:
            ...
            azimuth_deg = (
                torch.rand(self.batch_size) * (self.azimuth_range[1] - self.azimuth_range[0]) + self.azimuth_range[0]
            )
        azimuth = azimuth_deg * math.pi / 180

        # sample distances from a uniform distribution bounded by distance_range
        camera_distances: Float[Tensor, "..."] = (
            torch.rand(self.batch_size) * (self.camera_distance_range[1] - self.camera_distance_range[0])
        ...
        camera_positions: Float[Tensor, "..."] = torch.stack(
            [
        ...
        center: Float[Tensor, "..."] = torch.zeros_like(camera_positions)
        # default camera up direction as +z
        up: Float[Tensor, "..."] = torch.as_tensor([0, 0, 1], dtype=torch.float32)[None, :].repeat(self.batch_size, 1)

        # sample camera perturbations from a uniform distribution [-camera_perturb, camera_perturb]
        camera_perturb: Float[Tensor, "..."] = (
            torch.rand(self.batch_size, 3) * 2 * self.cfg.camera_perturb - self.cfg.camera_perturb
        )
        camera_positions = camera_positions + camera_perturb
        # sample center perturbations from a normal distribution with mean 0 and std center_perturb
        center_perturb: Float[Tensor, "..."] = torch.randn(self.batch_size, 3) * self.cfg.center_perturb
        center = center + center_perturb
        # sample up perturbations from a normal distribution with mean 0 and std up_perturb
        up_perturb: Float[Tensor, "..."] = torch.randn(self.batch_size, 3) * self.cfg.up_perturb
        up = up + up_perturb

        # sample fovs from a uniform distribution bounded by fov_range
        fovy_deg: Float[Tensor, "..."] = (
            torch.rand(self.batch_size) * (self.fovy_range[1] - self.fovy_range[0]) + self.fovy_range[0]
        )
        fovy = fovy_deg * math.pi / 180

        # sample light distance from a uniform distribution bounded by light_distance_range
        light_distances: Float[Tensor, "..."] = (
            torch.rand(self.batch_size) * (self.cfg.light_distance_range[1] - self.cfg.light_distance_range[0])
            + self.cfg.light_distance_range[0]
        )

        if self.cfg.light_sample_strategy == "dreamfusion":
            # sample light direction from a normal distribution with mean camera_position and std light_position_perturb
            light_direction: Float[Tensor, "..."] = F.normalize(
                camera_positions + torch.randn(self.batch_size, 3) * self.cfg.light_position_perturb,
                dim=-1,
            )
            # get light position by scaling light direction by light distance
            light_positions: Float[Tensor, "..."] = light_direction * light_distances[:, None]
        ...
        lookat: Float[Tensor, "..."] = F.normalize(center - camera_positions, dim=-1)
        right: Float[Tensor, "..."] = F.normalize(torch.cross(lookat, up), dim=-1)
        up = F.normalize(torch.cross(right, lookat), dim=-1)
        c2w3x4: Float[Tensor, "..."] = torch.cat(
            [torch.stack([right, up, -lookat], dim=-1), camera_positions[:, :, None]],
            dim=-1,
        )
        c2w: Float[Tensor, "..."] = torch.cat([c2w3x4, torch.zeros_like(c2w3x4[:, :1])], dim=1)
        c2w[:, 3, 3] = 1.0

        # get directions by dividing directions_unit_focal by focal length
        focal_length: Float[Tensor, "..."] = 0.5 * self.height / torch.tan(0.5 * fovy)
        directions: Float[Tensor, "..."] = self.directions_unit_focal[None, :, :, :].repeat(self.batch_size, 1, 1, 1)
        ...
        self.proj_mtx: Float[Tensor, "..."] = get_projection_matrix(
            fovy, self.width / self.height, 0.01, 100.0
        )  # FIXME: hard-coded near and far
        mvp_mtx: Float[Tensor, "..."] = get_mvp_matrix(c2w, self.proj_mtx)
        self.fovy = fovy
...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        elevation_deg: Float[Tensor, "..."] = torch.full_like(azimuth_deg, self.cfg.eval_elevation_deg)
        camera_distances: Float[Tensor, "..."] = torch.full_like(elevation_deg, self.cfg.eval_camera_distance)
        ...
        center: Float[Tensor, "..."] = torch.zeros_like(camera_positions)
        # default camera up direction as +z
        up: Float[Tensor, "..."] = torch.as_tensor([0, 0, 1], dtype=torch.float32)[None, :].repeat(
            self.cfg.eval_batch_size, 1
        )

        fovy_deg: Float[Tensor, "..."] = torch.full_like(elevation_deg, self.cfg.eval_fovy_deg)
        fovy = fovy_deg * math.pi / 180
        light_positions: Float[Tensor, "..."] = camera_positions
        ...
        # get directions by dividing directions_unit_focal by focal length
        focal_length: Float[Tensor, "..."] = 0.5 * self.cfg.eval_height / torch.tan(0.5 * fovy)
        directions_unit_focal = get_ray_directions(H=self.cfg.eval_height, W=self.cfg.eval_width, focal=1.0)
        directions: Float[Tensor, "..."] = directions_unit_focal[None, :, :, :].repeat(self.n_views, 1, 1, 1)
        directions[:, :, :, :2] = directions[:, :, :, :2] / focal_length[:, None, None, None]
