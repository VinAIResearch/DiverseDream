...
@threestudio.register("tsd-system")
class TSDSystem(BaseLift3DSystem):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        if self.cfg.hiper_path is not None:
            self.hiper_ckpt_path = sorted(glob.glob(f"{self.cfg.hiper_path}/*.pt"), key=sort_key)
            num_hiper = len(self.hiper_ckpt_path)
            assert num_hiper >= self.n_particles, "number of hiper checkpoint must larger than num particles"
            self.hiper_ckpt = [torch.load(self.hiper_ckpt_path[i]) for i in range(len(self.hiper_ckpt_path))]
            self.hiper_ckpt = self.hiper_ckpt[:num_hiper]
            [p.requires_grad_(False) for p in self.hiper_ckpt]
        else:
            self.hiper_ckpt = [None] * self.n_particles

    def forward(self, batch, idx) -> Dict[str, Any]:
        if self.cfg.stage == "geometry":
            render_out = self.renderer(idx=idx, **batch, render_rgb=False)
        else:
            render_out = self.renderer(idx=idx, **batch)
        return {
            **render_out,
        }
    ...
    def on_fit_start(self) -> None:
        super().on_fit_start()
        self.hiper_ckpt = [p.to(device=self.device) for p in self.hiper_ckpt]
            ...
            if self.cfg.visualize_samples:
                self.save_image_grid(
                    ...
                    [
                        {
                            "type": "rgb",
                            "img": self.guidance.sample(self.prompt_utils[i], **batch, seed=self.global_step)[0],
                            "kwargs": {"data_format": "HWC"},
                        },
                        {
                            "type": "rgb",
