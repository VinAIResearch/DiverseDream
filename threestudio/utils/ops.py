...
ValidScale = Union[Tuple[float, float], Num[Tensor, "..."]]


def scale_tensor(dat: Num[Tensor, "..."], inp_scale: ValidScale, tgt_scale: ValidScale):
...
def get_ray_directions(
    ...
    directions: Float[Tensor, "..."] = torch.stack([(i - cx) / fx, -(j - cy) / fy, -torch.ones_like(i)], -1)

    return directions


def get_rays(
    directions: Float[Tensor, "..."],
    c2w: Float[Tensor, "..."],
    keepdim=False,
    noise_scale=0.0,
    normalize=True,
...
def get_projection_matrix(
    fovy: Float[Tensor, "..."], aspect_wh: float, near: float, far: float
) -> Float[Tensor, "..."]:
...
def get_mvp_matrix(c2w: Float[Tensor, "..."], proj_mtx: Float[Tensor, "..."]) -> Float[Tensor, "..."]:
    # calculate w2c from c2w: R' = Rt, t' = -Rt * t
    # mathematically equivalent to (c2w)^-1
    w2c: Float[Tensor, "..."] = torch.zeros(c2w.shape[0], 4, 4).to(c2w)
...
def get_full_projection_matrix(c2w: Float[Tensor, "..."], proj_mtx: Float[Tensor, "..."]) -> Float[Tensor, "..."]:
...
def tet_sdf_diff(vert_sdf: Float[Tensor, "..."], tet_edges: Integer[Tensor, "..."]) -> Float[Tensor, "..."]:
...
def perpendicular_component(x: Float[Tensor, "..."], y: Float[Tensor, "..."]):
