    ...
    def update_step(self, epoch, global_step, on_load_weights=False):
        if self.n_masking_step <= 0 or global_step is None:
            ...
            self.mask = (
                ...
                - torch.cos(
                    math.pi
                    * (global_step / self.n_masking_step * self.N_freqs - torch.arange(0, self.N_freqs)).clamp(0, 1)
                )
            ) / 2.0
        ...
        if self.update_occ_grid and not isinstance(self.frame_time, float):
            ...
            if (self.static or not self.training) and self.frame_time is None:
                ...
                if self.frame_time is None:
                    ...
                    frame_time = self.frame_time
                frame_time = (torch.ones((self.num_frames, 1), device=x.device, dtype=x.dtype) * frame_time).expand(
                    x.shape[0], 1
                )
...
class ProgressiveBandHashGrid(nn.Module, Updateable):
    ...
    def update_step(self, epoch, global_step, on_load_weights=False):
        current_level = min(
            self.start_level + max(global_step - self.start_step, 0) // self.update_steps,
            self.n_level,
        )
...
class CompositeEncoding(nn.Module, Updateable):
    ...
    def forward(self, x, *args):
        return (
            ...
            else torch.cat([x * self.xyz_scale + self.xyz_offset, self.encoding(x, *args)], dim=-1)
        )
