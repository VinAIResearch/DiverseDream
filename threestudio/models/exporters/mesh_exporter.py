...
class MeshExporter(Exporter):
    ...
    def export_obj_with_mtl(self, mesh: Mesh) -> List[ExporterOutput]:
        ...
        if self.cfg.save_texture:
            ...
            # rasterize
            rast, _ = self.ctx.rasterize_one(uv_clip4, mesh.t_tex_idx, (self.cfg.texture_size, self.cfg.texture_size))
            ...
            gb_pos, _ = self.ctx.interpolate_one(mesh.v_pos, rast[None, ...], mesh.t_pos_idx)
            gb_pos = gb_pos[0]
            ...
            threestudio.info("Perform UV padding on texture maps to avoid seams, may take a while ...")

            if "albedo" in mat_out:
                params["map_Kd"] = uv_padding(mat_out["albedo"])
            else:
                threestudio.warn("save_texture is True but no albedo texture found, using default white texture")
    ...
    def export_obj(self, mesh: Mesh) -> List[ExporterOutput]:
        ...
        if self.cfg.save_texture:
            ...
            if "albedo" in mat_out:
                ...
                threestudio.warn("save_texture is True but no albedo texture found, not saving vertex color")
