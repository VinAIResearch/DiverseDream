...
class ImportanceEstimator(AbstractEstimator):
    ...
    def sampling(
        ...
        assert len(prop_sigma_fns) == len(prop_samples), (
            "The number of proposal networks and the number of samples " "should be the same."
        )
        cdfs = torch.cat(
            [
        ...
        t_vals_fine = _transform_stot(sampling_type, intervals.vals, near_plane, far_plane)
...
def _transform_stot(
    ...
    def ico(s):
        return _icontract_fn(s * s_max + (1 - s) * s_min)
