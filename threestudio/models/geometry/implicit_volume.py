...
class ImplicitVolume(BaseImplicitGeometry):
    ...
    def get_activated_density(
        self, points: Float[Tensor, "..."], density: Float[Tensor, "..."]
    ) -> Tuple[Float[Tensor, "..."], Float[Tensor, "..."]]:
        density_bias: Union[float, Float[Tensor, "..."]]
        ...
        raw_density: Float[Tensor, "..."] = density + density_bias
        density = get_activation(self.cfg.density_activation)(raw_density)
        return raw_density, density

    def forward(self, points: Float[Tensor, "..."], output_normal: bool = False) -> Dict[str, Float[Tensor, "..."]]:
        ...
        if output_normal:
            if self.cfg.normal_type == "finite_difference" or self.cfg.normal_type == "finite_difference_laplacian":
                ...
                if self.cfg.normal_type == "finite_difference_laplacian":
                    offsets: Float[Tensor, "..."] = torch.as_tensor(
                        [
                            [eps, 0.0, 0.0],
                            [-eps, 0.0, 0.0],
                            [0.0, eps, 0.0],
                    ...
                    points_offset: Float[Tensor, "..."] = (points_unscaled[..., None, :] + offsets).clamp(
                        -self.cfg.radius, self.cfg.radius
                    )
                    density_offset: Float[Tensor, "..."] = self.forward_density(points_offset)
    ...
    def forward_density(self, points: Float[Tensor, "..."]) -> Float[Tensor, "..."]:
    ...
    def forward_field(
        self, points: Float[Tensor, "..."]
    ) -> Tuple[Float[Tensor, "..."], Optional[Float[Tensor, "..."]]]:
        if self.cfg.isosurface_deformable_grid:
            threestudio.warn(f"{self.__class__.__name__} does not support isosurface_deformable_grid. Ignoring.")
        density = self.forward_density(points)
        return density, None

    def forward_level(self, field: Float[Tensor, "..."], threshold: float) -> Float[Tensor, "..."]:
        return -(field - threshold)

    def export(self, points: Float[Tensor, "..."], **kwargs) -> Dict[str, Any]:
        out: Dict[str, Any] = {}
        if self.cfg.n_feature_dims == 0:
            return out
        points_unscaled = points
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False) -> None:
        if self.cfg.anneal_density_blob_std_config is not None:
            ...
            if global_step >= min_step and global_step <= max_step:
                ...
                self.density_blob_std = start_val + (global_step - min_step) * (end_val - start_val) / (
                    max_step - min_step
                )
