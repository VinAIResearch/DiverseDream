...
class Mesh:
    def __init__(self, v_pos: Float[Tensor, "..."], t_pos_idx: Integer[Tensor, "..."], **kwargs) -> None:
        self.v_pos: Float[Tensor, "..."] = v_pos
        self.t_pos_idx: Integer[Tensor, "..."] = t_pos_idx
        self._v_nrm: Optional[Float[Tensor, "..."]] = None
        self._v_tng: Optional[Float[Tensor, "..."]] = None
        self._v_tex: Optional[Float[Tensor, "..."]] = None
        self._t_tex_idx: Optional[Float[Tensor, "..."]] = None
        self._v_rgb: Optional[Float[Tensor, "..."]] = None
        self._edges: Optional[Integer[Tensor, "..."]] = None
    ...
    def _unwrap_uv(self, xatlas_chart_options=None, xatlas_pack_options=None):
        if xatlas_chart_options is None:
            xatlas_chart_options = {}
        if xatlas_pack_options is None:
            xatlas_pack_options = {}
        ...
        vmapping = (
            torch.from_numpy(vmapping.astype(np.uint64, casting="same_kind").view(np.int64))
            .to(self.v_pos.device)
            .long()
        )
        uvs = torch.from_numpy(uvs).to(self.v_pos.device).float()
        indices = (
            torch.from_numpy(indices.astype(np.uint64, casting="same_kind").view(np.int64))
            .to(self.v_pos.device)
            .long()
        )
        return uvs, indices

    def unwrap_uv(self, xatlas_chart_options=None, xatlas_pack_options=None):
        if xatlas_chart_options is None:
            xatlas_chart_options = {}
        if xatlas_pack_options is None:
            xatlas_pack_options = {}
    ...
    def normal_consistency(self) -> Float[Tensor, "..."]:
        edge_nrm: Float[Tensor, "..."] = self.v_nrm[self.edges]
