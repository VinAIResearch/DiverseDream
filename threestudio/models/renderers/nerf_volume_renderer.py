...
class NeRFVolumeRenderer(VolumeRenderer):
    ...
    def configure(
        ...
        if self.cfg.estimator == "occgrid":
            ...
            self.prop_net = create_network_with_input_encoding(**self.cfg.proposal_network_config)
            self.prop_optim = parse_optimizer(self.cfg.prop_optimizer_config, self.prop_net)
            self.prop_scheduler = (
                parse_scheduler_to_instance(self.cfg.prop_scheduler_config, self.prop_optim)
                if self.cfg.prop_scheduler_config is not None
                else None
            )
            self.estimator = nerfacc.PropNetEstimator(self.prop_optim, self.prop_scheduler)

            def get_proposal_requires_grad_fn(target: float = 5.0, num_steps: int = 1000):
                def f(s):
                    return min(s / num_steps, 1.0) * target
            ...
            raise NotImplementedError("Unknown estimator, should be one of ['occgrid', 'proposal', 'importance'].")
    ...
    def forward(
        ...
        **kwargs
        ...
        rays_o_flatten: Float[Tensor, "..."] = rays_o.reshape(-1, 3)
        rays_d_flatten: Float[Tensor, "..."] = rays_d.reshape(-1, 3)
        light_positions_flatten: Float[Tensor, "..."] = (
            light_positions.reshape(-1, 1, 1, 3).expand(-1, height, width, -1).reshape(-1, 3)
        )
        ...
        if self.cfg.estimator == "occgrid":
            ...
            def prop_sigma_fn(
                t_starts: Float[Tensor, "..."],
                t_ends: Float[Tensor, "..."],
                proposal_network,
            ):
                t_origins: Float[Tensor, "..."] = rays_o_flatten.unsqueeze(-2)
                t_dirs: Float[Tensor, "..."] = rays_d_flatten.unsqueeze(-2)
                positions: Float[Tensor, "..."] = t_origins + t_dirs * (t_starts + t_ends)[..., None] / 2.0
                aabb_min, aabb_max = self.bbox[0], self.bbox[1]
                positions = (positions - aabb_min) / (aabb_max - aabb_min)
                selector = ((positions > 0.0) & (positions < 1.0)).all(dim=-1)
                density_before_activation = (
                    proposal_network(positions.view(-1, 3)).view(*positions.shape[:-1], 1).to(positions)
                )
                density: Float[Tensor, "..."] = (
                    get_activation("shifted_trunc_exp")(density_before_activation) * selector[..., None]
                )
            ...
            ray_indices = (
                torch.arange(n_rays, device=rays_o_flatten.device).unsqueeze(-1).expand(-1, t_starts_.shape[1])
            )
        ...
        if self.training:
            ...
            rgb_fg_all = self.material(
                viewdirs=t_dirs, positions=positions, light_positions=t_light_positions, **geo_out, **kwargs
            )
        ...
        opacity: Float[Tensor, "..."] = nerfacc.accumulate_along_rays(
            weights[..., 0], values=None, ray_indices=ray_indices, n_rays=n_rays
        )
        depth: Float[Tensor, "..."] = nerfacc.accumulate_along_rays(
            weights[..., 0], values=t_positions, ray_indices=ray_indices, n_rays=n_rays
        )
        comp_rgb_fg: Float[Tensor, "..."] = nerfacc.accumulate_along_rays(
        ...
        z_mean: Float[Tensor, "..."] = nerfacc.accumulate_along_rays(
        ...
        if self.training:
            ...
            if "normal" in geo_out:
                if self.cfg.return_comp_normal:
                    comp_normal: Float[Tensor, "..."] = nerfacc.accumulate_along_rays(
                        weights[..., 0],
