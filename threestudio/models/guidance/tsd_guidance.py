...
@threestudio.register("stable-diffusion-tsd-guidance")
class StableDiffusionTSDGuidance(BaseModule):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        if self.cfg.pretrained_model_name_or_path == self.cfg.pretrained_model_name_or_path_lora:
        ...
        self.camera_embedding = ToWeightsDType(TimestepEmbedding(16, 1280), self.weights_dtype).to(self.device)
        ...
        if self.cfg.use_lora:
            ...
            self.learnable_text = torch.nn.Parameter(torch.randn(1, int(self.cfg.token_len), 768))
    ...
    def sample_lora(
        ...
        **kwargs,
        ...
        if self.cfg.camera_condition_type == "extrinsics":
            ...
            raise ValueError(f"Unknown camera_condition_type {self.cfg.camera_condition_type}")
    ...
    @torch.cuda.amp.autocast(enabled=False)
    def encode_images(self, imgs: Float[Tensor, "..."]) -> Float[Tensor, "..."]:
        input_dtype = imgs.dtype
        imgs = imgs * 2.0 - 1.0
        posterior = self.vae.encode(imgs.to(self.weights_dtype)).latents
    ...
    def compute_grad_vsd(
        ...
        with torch.no_grad():
            ...
            use_lora = {"scale": 1.0} if self.cfg.use_lora else None
            noise_pred_est = self.forward_unet(
                ...
                cross_attention_kwargs=use_lora,  # {"scale": 1.0 use lora} else None,
            )
    ...
    def train_lora(
        ...
        noise_pred = self.forward_unet(
            ...
            class_labels=camera_condition.view(B, -1).repeat(self.cfg.lora_n_timestamp_samples, 1),
            cross_attention_kwargs=use_lora,
        )
        return F.mse_loss(noise_pred.float(), target.float(), reduction="mean")

    def get_latents(self, rgb_BCHW: Float[Tensor, "..."], rgb_as_latents=False) -> Float[Tensor, "..."]:
        if rgb_as_latents:
    ...
    def forward(
        ...
        **kwargs,
        ...
        text_embeddings_vd = prompt_utils.get_text_embeddings(
            ...
            azimuth,
            camera_distances,
            num_train_step=self.num_train_timesteps,
            view_dependent_prompting=self.cfg.view_dependent_prompting,
            hiper_guidance=hiper_guidance,
        )

        # input text embeddings, view-independent (lora)
        text_embeddings = prompt_utils.get_text_embeddings(
            elevation,
            azimuth,
            camera_distances,
        ...
        grad = self.compute_grad_vsd(
            latents, text_embeddings_vd.detach().clone(), text_embeddings.detach().clone(), camera_condition, t=t
        )
