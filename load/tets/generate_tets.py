...
def generate_tetrahedron_grid_file(res=32, root=".."):
    frac = 1.0 / res
    command = (
        f"cd {root}; ./quartet meshes/cube.obj {frac} meshes/cube_{res}_tet.tet -s meshes/cube_boundary_{res}.obj"
    )
    os.system(command)
...
for res in [300, 350, 400]:
    ...
    convert_from_quartet_to_npz(os.path.join(root, f"meshes/cube_{res}_tet.tet"), npzfile=f"{res}_tets")
