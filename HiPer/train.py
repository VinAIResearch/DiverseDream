...
def main():
    ...
    pipe = StableDiffusionPipeline.from_pretrained(
        args.pretrained_model_name, scheduler=ddim_scheduler, torch_dtype=torch.float16
    ).to("cuda")
    num_samples = 1
    guidance_scale = 7.5
        ...
        for step in pbar:
            ...
            if step % 100 == 0:
                ...
                save_name = os.path.join(
                    args.output_dir,
                    "training_results_seed{}_step{}.png".format(args.seed, step),
                )
                inf_save(
                    inf_images,
                    ["source_img", "[src, hper]", "[tgt, hper]", "[tgt]"],
                    save_name,
                )
                del images

                torch.save(
                    hiper_embeddings.cpu(),
                    os.path.join(args.output_dir, "hiper_embeddings_step{}.pt".format(step)),
                )
